...
class PortfolioManager {
    ...
    // ===== TYPING ANIMATION - FIXED TO PREVENT LAYOUT SHIFTS =====
    setupTypingAnimation() {
        const typedElement = document.querySelector('.typedText');
        if (!typedElement) return;

        // Create a container with fixed dimensions to prevent layout shifts
        const container = typedElement.parentElement;
        if (container) {
            // Apply styles to prevent layout shifts
            ...
            // Ensure the typed element is properly contained
